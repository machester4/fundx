...
- Daily-per-fund cap (Phase 4): `fund.budget.dailyCapUsd` → `global.budget.dailyCapUsd` → default $5/day (see `resolveDailyCapUsd` in `src/services/session.service.ts`). When today's `cost_usd` sum across `state/session_log.jsonl` reaches the cap, further sessions are skipped with status `skipped_daily_cap` until 00:00 UTC. One-shot Telegram alert per fund per day. See `docs/operations.md` runbook.
