...
│           └── skills/            # 9 trading skills (generated on fund creation)
...
- Daily-per-fund cap (Phase 4, raised in Phase 5c): `fund.budget.dailyCapUsd` → `global.budget.dailyCapUsd` → default $90/day (scaled for the Opus 4.8 default in 2026-06; was $20; users can lower via fund or global config). See `resolveDailyCapUsd` in `src/services/session.service.ts`. When today's `cost_usd` sum across `state/session_log.jsonl` reaches the cap, further sessions are skipped with status `skipped_daily_cap` until 00:00 UTC. One-shot OS notification per fund per day. See `docs/operations.md` runbook.
...
**Where skills are defined:** `src/skills.ts` — `BUILTIN_SKILLS` (9 fund skills) + `WORKSPACE_SKILL` (create-fund). Generated via `ensureFundSkillFiles()` on fund creation and `ensureWorkspaceSkillFiles()` on init.
